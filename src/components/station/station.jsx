...
const Station = () => {
	...
	const createStation = (e) => {
		...
		axios
			.post(
				url + "/Stations/create",
				{ id, stationName, companyId, stationCode },
				{
					headers: {
	...
	const getData = () => {
		axios
			.get(url + "/Stations/get-station-by-company/" + companyId, {
	...
	const stationNumber = () => {
		axios
			.get(url + "/Stations/get-station-count/" + companyId, {
				headers: { Authorization: `Bearer ${token}` },
			})
			.then((res) => {
	...
	return (
		<div>
			...
			<hr></hr>
			<button className="border" onClick={stationNumber}>
				Total stations under company
			</button>
